...
(async () => {
    ...
    try {
        ...
        page.on('request', async (req) => {
            ...
            if (req.url().includes('video/720.m3u8') || req.url().includes('video/1080.m3u8')) {
                console.log("\nResquest => " + req.url());

                request(req.url(), async function (error, response, m3u8Content) {

                    if (error != NaN && error != null && error) {
                        console.error("\nErro no resquest => " + error);
                    }
                    ...
                    fs.writeFile(m3u8FilePath, m3u8Content, function (err) {
                        ...
                        //const cmd = "echo 'Gerando o video .mp4' && sh gerar.sh " + filegenerate;
                ...
                //console.log("\n");
                //console.log("*****************************************");
                //console.log("****** ITEM PRECISO NAO ENCONTRADO ******");
                //console.log("*****************************************");
                //console.log("\n");
            }
